...
require_once __DIR__ . '/../db.php'; // gives $pdo (and $conn if needed)
...
$fetchError = '';
try {
    $stmt = $pdo->query('SELECT id, agent_name, agent_address, party_name, agent_email, dob, profile_picture, party_logo FROM agents ORDER BY id DESC');
    $agents = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $agentCount = count($agents);
} catch (Throwable $e) {
    $fetchError = 'Unable to load agents right now.'; // hide internal error
}
?>
...
<body class="bg-gray-50 min-h-screen">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
            ...
            <?php if ($fetchError): ?>
                <div class="p-6 text-red-600 text-sm"><?= safe($fetchError) ?></div>
            <?php elseif ($agentCount > 0): ?>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <?php foreach ($agents as $row):
                                ...
                                // Normalize stored file paths (DB stores like 'uploads/filename')
                                $profileRel = trim($row['profile_picture'] ?? '');
                                $logoRel    = trim($row['party_logo'] ?? '');
                                $profileFs  = $profileRel ? realpath(__DIR__ . '/../' . $profileRel) : '';
                                $logoFs     = $logoRel ? realpath(__DIR__ . '/../' . $logoRel) : '';
                                $profileOk  = $profileFs && is_file($profileFs) && str_starts_with($profileFs, realpath(__DIR__ . '/../uploads'));
                                $logoOk     = $logoFs && is_file($logoFs) && str_starts_with($logoFs, realpath(__DIR__ . '/../uploads'));
                            ?>
                                <tr class="hover:bg-gray-50 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <?php endif; ?>
                                            <div class="ml-3">
                                                <div class="text-xs text-gray-500">ID: #<?= safe($row['id']) ?></div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <!-- Agent Info Column -->
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-medium text-gray-900"><?= safe($row['agent_name']) ?></div>
                                        <div class="text-sm text-gray-500"><?= safe($row['agent_address']) ?></div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center">
                                            ...
                                            <?php endif; ?>
                                            <div>
                                                <div class="text-sm font-medium text-gray-900"><?= safe($row['party_name']) ?></div>
                                    ...
                                    <!-- Contact Info Column -->
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-900"><?= safe($row['agent_email']) ?></div>
                                        ...
                                        <?php if (!empty($age)): ?>
                                            ...
                                            <?php if (!empty($row['dob'])): ?>
                                                <div class="text-xs text-gray-500 mt-1">
                                                    DOB: <?= safe(date('M d, Y', strtotime($row['dob']))) ?>
                                                </div>
                                            <?php endif; ?>
                                        <?php else: ?>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-3">
                                            <!-- View Details Button -->
                                            <button onclick="viewAgent(<?= (int)$row['id'] ?>)" class="text-blue-600 hover:text-blue-800 transition-colors duration-200" title="View Details">
                                            ...
                                            <!-- Edit Button -->
                                            <a href="update_agent.php?id=<?= (int)$row['id'] ?>" class="text-green-600 hover:text-green-800 transition-colors duration-200" title="Edit Agent">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="mt-8 flex items-center justify-between">
            ...
            <div class="flex items-center space-x-2 text-sm text-gray-500">
                ...
                <span class="font-medium"><?= safe(date('M d, Y H:i')) ?></span>
            </div>
        </div>
    </div>
