...
// First get total votes
$total_sql = "SELECT COUNT(*) AS total FROM votes";
...
// Get votes grouped by party
$sql = "SELECT p.party_name, p.party_logo, p.politician_name, COUNT(v.vote_id) AS total_votes
        FROM votes v
        JOIN parties p ON v.party_id = p.party_id
        GROUP BY p.party_id, p.party_name, p.politician_name
...
if ($result->num_rows > 0) {
    $result->data_seek(0); // Reset to first row
    $winner_info = $result->fetch_assoc();
    $result->data_seek(0); // Reset again for the main loop
}
?>
...
<body class="bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 min-h-screen">
    ...
    <div class="relative z-10 container mx-auto px-4 py-8">
        ...
        <div class="text-center mb-12 animate-fade-in">
            ...
            <p class="text-blue-200 text-xl mb-6">Real-time voting statistics and analysis</p>
        ...
        <div class="grid md:grid-cols-4 gap-6 mb-12">
            ...
            <div class="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6 text-center animate-slide-up">
                ...
                <div class="text-3xl font-bold text-white mb-1 animate-count-up"><?php echo number_format($total_votes); ?></div>
            ...
            <div class="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6 text-center animate-slide-up" style="animation-delay: 0.1s;">
                ...
                <div class="text-3xl font-bold text-white mb-1"><?php echo $result->num_rows; ?></div>
                <div class="text-purple-200 text-sm">Participating Parties</div>
            </div>
            
            <!-- Winner -->
            <?php if ($winner_info && $total_votes > 0): ?>
            <div class="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6 text-center animate-slide-up" style="animation-delay: 0.2s;">
                ...
                <div class="text-xl font-bold text-white mb-1"><?php echo htmlspecialchars($winner_info['party_name']); ?></div>
            ...
            <div class="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6 text-center animate-slide-up" style="animation-delay: 0.3s;">
                ...
                <div class="text-3xl font-bold text-white mb-1">
                    <?php echo $total_votes > 0 ? round(($winner_info['total_votes'] / $total_votes) * 100, 1) : 0; ?>%
                </div>
            ...
            <div class="overflow-x-auto">
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-white/10">
                        ...
                        ?>
                        <tr class="hover:bg-white/5 transition duration-300 <?php echo $isWinner ? 'winner-glow bg-green-500/5' : ''; ?>">
                            <td class="px-8 py-6">
                            ...
                            <td class="px-8 py-6">
                                <div class="flex items-center">
                                    <div class="w-12 h-12 rounded-full overflow-hidden mr-4 border-2 border-white/20">
                                        <img src="<?php echo htmlspecialchars($row['party_logo']); ?>" 
                                             alt="<?php echo htmlspecialchars($row['party_name']); ?>" 
                            ...
                            <td class="px-8 py-6">
                                <div class="text-blue-200 font-medium">
                                    <?php echo htmlspecialchars($row['politician_name']); ?>
                                </div>
                            </td>
                            <td class="px-8 py-6">
                                <div class="text-white font-bold text-xl">
                                    <?php echo number_format($row['total_votes']); ?>
                                </div>
                            </td>
                            <td class="px-8 py-6">
                            ...
                            <td class="px-8 py-6">
                                <div class="w-full bg-white/10 rounded-full h-3 overflow-hidden">
                                    <div class="vote-bar h-full rounded-full <?php echo $colors[($rank-1) % count($colors)]; ?>" 
                                         style="width: <?php echo $percentage; ?>%"></div>
                                </div>
                                <div class="text-xs text-blue-200 mt-1">
                                    <?php echo $percentage; ?>% of total votes
                                </div>
                            </td>
                        </tr>
                        <?php 
